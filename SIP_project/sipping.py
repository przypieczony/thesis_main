...
def gen_request(template_vars, options):
	...
	for i in xrange(options.count):
		...
		for k in template_vars.keys():
			if k.startswith("."):
				template_vars[k[1:]] = eval(template_vars[k]) # executes python code from argument which start with "."
...
def main():
	...
	if options.simulate:
		...
		options.source_ip = random_loopback
		options.dest_ip = "127.0.0.1"
